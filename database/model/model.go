...
// ClientAutoreset stores traffic auto-reset configuration for clients
// Using a separate table for performance - only clients with reset enabled are stored
type ClientAutoreset struct {
	Id              uint  `json:"id" form:"id" gorm:"primaryKey;autoIncrement"`
	ClientId        uint  `json:"clientId" form:"clientId" gorm:"uniqueIndex"`
	ResetMode       int   `json:"resetMode" form:"resetMode"`               // 1=monthly, 2=periodic (every N days)
	...
	CreatedAt       int64 `json:"createdAt" form:"createdAt"`               // Client creation timestamp (for calculating reset day)
}
...
type TrafficHistory struct {
	...
	EndTime    int64  `json:"endTime" form:"endTime" gorm:"index"`     // Period end timestamp (reset time)
